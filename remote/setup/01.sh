...
# Prompt to enter a password for the PostgreSQL cinedata user (rather than hard-coding
# a password in this script).
read -p "Enter password for cinedata DB user: " DB_PASSWORD
...
# installed support for all locales.
...
# Copy the SSH keys from the root user to new user.
...
# Setup the cinedata DB and create a user account with the password entered earlier.
sudo -i -u postgres psql -c "CREATE DATABASE cinedata"
sudo -i -u postgres psql -d cinedata -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d cinedata -c  "CREATE ROLE cinedata WITH LOGIN PASSWORD '${DB_PASSWORD}'"

# Add a DSN for connecting to the greenlight database to the system-wide environment
# variables in the /etc/environment file.
echo "CINEDATA_DB_DSN='postgres://cinedata:${DB_PASSWORD}@localhost/cinedata'" >> /etc/environment
